...
function mostrarMesesDesaprobados(
  notasAlumno: number[],
  meses: string[]
): string {
  let indice: number;
...
function buscarMenorNota(notasAlumno: number[], meses: string[]): any {
...
function buscarMayorNota(notasAlumno: number[], meses: string[]): any {
  let notaMax: number = 0;
  let mesMax: string = " ";
  for (indice = 0; indice < meses.length; indice++) {
...
// Función promedio
function calcularPromedio(notasAlumno: number[]): number {
  let suma: number = 0;
...
console.log("El promedio del alumno es " + calcularPromedio(notasAlumno));
